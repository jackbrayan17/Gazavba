...
function ensureSampleBackgrounds() {
  ...
  backgrounds.forEach(name => {
    ...
    if (!fs.existsSync(filePath)) {
      // Fichier factice (1x1 px transparent ou couleur)
...
function ensureSampleMedia() {
  const files = ['sample-image.jpg', 'sample-video.mp4'];
...
async function clearSeedData() {
  const seedEmails = [
    'alice@gazavba.local',
    'boris@gazavba.local',
    'carmen@gazavba.local',
    'diego@gazavba.local',
    'estelle@gazavba.local',
  ];

  const placeholders = seedEmails.map(() => '?').join(', ');
  const existing = await all(`SELECT id, email FROM users WHERE email IN (${placeholders})`, seedEmails);

  if (!existing.length) {
    return false;
  }

  const ids = existing.map(u => String(u.id));
  const idPlaceholders = ids.map(() => '?').join(', ');

  // Chats involving seed users (direct or group)
  const directChats = await all(
    `SELECT id FROM chats WHERE type = 'direct' AND (user1Id IN (${idPlaceholders}) OR user2Id IN (${idPlaceholders}))`,
    [...ids, ...ids]
  );
  const groupChats = await all(`SELECT id FROM chats WHERE name = ?`, ['Équipe Gazavba']);
  const participantChats = await all(
    `SELECT DISTINCT chatId AS id FROM chat_participants WHERE userId IN (${idPlaceholders})`,
    ids
  );

  const chatIds = Array.from(new Set([...directChats, ...groupChats, ...participantChats].map(c => c.id)));
  const chatPlaceholders = chatIds.map(() => '?').join(', ');

  if (chatIds.length) {
    // Delete reads for messages in these chats
    await run(
      `DELETE mr FROM message_reads mr
         JOIN messages m ON mr.messageId = m.id
       WHERE m.chatId IN (${chatPlaceholders})`,
      chatIds
    );

    await run(`DELETE FROM statuses WHERE userId IN (${idPlaceholders})`, ids);
    await run(`DELETE FROM messages WHERE chatId IN (${chatPlaceholders})`, chatIds);
    await run(`DELETE FROM user_chat_settings WHERE chatId IN (${chatPlaceholders})`, chatIds);
    await run(`DELETE FROM chat_participants WHERE chatId IN (${chatPlaceholders})`, chatIds);
    await run(`DELETE FROM chats WHERE id IN (${chatPlaceholders})`, chatIds);
  }

  // Any leftover rows tied to the users
  await run(`DELETE FROM statuses WHERE userId IN (${idPlaceholders})`, ids);
  await run(`DELETE FROM messages WHERE senderId IN (${idPlaceholders})`, ids);
  await run(`DELETE FROM user_chat_settings WHERE userId IN (${idPlaceholders})`, ids);
  await run(`DELETE FROM chat_participants WHERE userId IN (${idPlaceholders})`, ids);
  await run(`DELETE FROM users WHERE id IN (${idPlaceholders})`, ids);

  console.log('Anciennes données de seed supprimées.');
  return true;
}

async function seedDatabase() {
  try {
    console.log('Seeding database... (December 06, 2025)');
    ...
    const rawUsers = [
      {
        ...
        bio: 'Développeuse backend passionnée par Node.js et MySQL. Basée Yaoundé.',
    ...
    // Attente que tous les utilisateurs soient créés
    ...
    const statuses = [
      ...
      { userId: created[3]?.id, type: 'video', content: 'Mini démo', mediaUrl: videoUrl, expiresAt },
    ].filter(s => s.userId);
